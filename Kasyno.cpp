...
void Kasyno::dodajBota(int _odwaga) {
	...
	if (iloscBotow == 0) {
		iloscBotow = 1;
		rozmiarTab++;
		listaBotow = new GraczKomputerowy[iloscBotow];
		...
		GraczKomputerowy *temp = nullptr;
		iloscBotow++;
		rozmiarTab++;
		temp = new GraczKomputerowy[iloscBotow];
		for (int i = 0; i < iloscBotow - 1; ++i) {
			temp[i].ustawNazwe(i);
			temp[i].ustawOdwage(listaBotow[i].getOdwaga());
		}
		...
		for (int i = 0; i < iloscBotow; ++i) {
			...
			listaBotow[i].ustawOdwage(temp[i].getOdwaga());
